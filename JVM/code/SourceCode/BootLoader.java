...
/**
  找到资源并进行打包置入到启动类中.或者是通过指定参数`-Xbootclasspath`对启动类路径进行设置
 参数列表:
 @UNNAMED_MODULE 匿名模块的启动类加载器
 @JAVA_HOME JavaHome
 @SERVICES_CATALOG 服务目录
 @CLASS_LOADER_VALUE_MAP 启动类加载器映射表
 */
...
public class BootLoader {
    ...
    /**
     加载给定模块下给定名称的类对象到启动类加载器中.如果寻找不到则返回null
     */
    ...
    /**
     如果给定名称的包位置由系统启动类加载,则返回给定包名称的位置,如果不存在则返回null

     这个位置可以是运行时的位置或者分解的位置,还可以是启动类添加的路径(通过-Xbootclasspath进行设置)
     */
