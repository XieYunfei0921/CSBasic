...
+ 双亲委派机制
  ...
  2. 扩展类加载器
     ...
     这个类加载器是在`sun.misc.Launcher$ExtClassLoader`中以java代码的形式实现,负载加载`<JAVA_HOME>/lib/ext`目录,或者被`java.ext.dirs`系统变量所指定路径中的所有类库.java 9之后被**模块化**所取代.由于是java代码实现,可以在程序中使用扩展类及咋去加载class文件.

  3. 应用程序类加载器

     这个类加载器有`AppClassLoader`实现,由于是ClassLoader的getSystemClassLoader犯法的返回值,也被称作**系统类加载器**.负责加载**类路径**所有类库.

     <img src="E:\截图文件\双亲委派模型.png" style="zoom:67%;" />

     上述层次关系称作类加载器的**双亲委派模型**.处理顶层之外其余的类加载器都应当有父类加载器.但是这里的父子关系不是继承,而是使用组合关系复用父加载器.
...
+ 模块化下的类加载器
  ...
  > **平台类加载器**和**应用程序加载器**不在派生`java.net.URL.ClassLoader`,如果有程序依赖这个继承关系,或者依赖了相关的方法.可能导致JDK 9+版本中崩溃.现在的**启动类加载器**,**平台类加载器**和**应用程序加载器**全部继承`dk.internal.loader.BuiltinClassLoader `

  JDK 9+的双亲委派模型:

  <img src="E:\截图文件\jdk9+双亲委派模型.png" style="zoom:67%;" />
