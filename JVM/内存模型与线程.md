...
5. 原子性,可见性与有序性
   ...
   + 可见性
     ...
     + 同步块的可见性是由对一个关键进行unlock操作之前,必须将变量同步回主存中决定的.
   ...
   + 有序性
     ...
     所有操作在本线程内观察都是有序的,在拎一个线程观察都是无须的.
...
1.  线程的实现
   ...
   + 内核线程
     ...
     **内核线程**是由操作系统直接支持的操作系统内核支持的线程,这种线程由内核进行线程切换,内核通过操作**调度器**对线程进行调度.并负责将线程任务映射到各个处理器上.
     ...
     <img src="E:\截图文件\LWP于KLT关系.png" style="zoom:67%;" />

   + 用户进程

     广义上来说,只要不是内核线程,就是用户线程.

     狭义上来说用户线程是值完全建立在用户空间的线程库,系统内核感知不到用户线程的存在.用户线程的建立,同步,销毁和调度完全在用户态下执行,不需要内核参与.操作快速且消耗低.
...
3.  状态转换
   ...
   <img src="E:\截图文件\线程切换图.png" style="zoom:67%;" />
